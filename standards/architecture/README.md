...
- **イメージのビルドはCIで行い、レジストリへpushする。** 本番・ステージングはpullして起動する。サーバ上でのビルドを通常運用にしない。サーバでビルドする構成では、ソース・ビルドツールチェーン・レジストリの認証情報をサーバへ置くことになり、テストしたイメージと動いているイメージが同一である保証も得られない。
